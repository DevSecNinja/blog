---
title: 'Azure - Deploy and automatically domain join a VM with Azure Automation Runbooks'
date: 2015-02-18T19:32:59+00:00
draft: false
...
**Don't forget to update the domain in the Add-Computer part.** To-Do list: - Custom static IP as variable.
    ...
    InlineScript
    {
        ...
        if ($Session) {
            ...
            Invoke-command -Session $Session -ArgumentList $Using:DomainCred -ScriptBlock {
                param($cred)
                Add-Computer -DomainName "corp.contoso.com" -DomainCredential $cred
